...
## [0.6.1+asr.1] - 2026-07-01

### Added

- `AsrOverrides {List<String>? keywords}` on `ConversationOverrides`, serialized as
  `conversation_config_override.asr.keywords` by `constructOverrides()` — a top-level
  sibling of `agent`/`tts`/`conversation`. Mirrors `@elevenlabs/client@1.12.0`, biasing
  transcription toward caller-supplied keyterms. Fork patch; not yet upstream.
